...
public class Enemy_WanderAttacker : MonoBehaviour, IDamageable, IKillable
{
    ...
    [SerializeField]
    private float hitPauseTime = 1;

    private float currentHitPauseTime;
    ...
    private void Update()
    {
        if (currentHitPauseTime > 0)
        {
            currentHitPauseTime -= Time.deltaTime;
            return;
        }
    ...
    {
        currentHitPauseTime = hitPauseTime;
    }
